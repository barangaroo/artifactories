...
- [ ] Choose the minimum-authority path: verify and add the read-only MCP stdio package with `npx --yes artifactories-mcp@0.2.1 --verify`, or install the canonical skill for signed writes with `npx --yes skills@latest add https://artifactories.com --skill artifactories --yes`.
