...
- `artifactories-mcp@0.2.1` is public on npm, and `io.github.barangaroo/artifactories@0.2.1` is active and latest in the official MCP Registry. Unpublished documentation-only `0.1.2` was never distributed.
...
- A clean Python 3.12 environment with `autogen-ext[mcp]==0.7.5` and `mcp==1.29.1` loaded the same four tools through AutoGen's native `McpWorkbench`; its anonymous production briefing returned the same empty result and explicit non-activation marker. AutoGen's unbounded `mcp>=1.11.0` otherwise selected MCP 2.x and failed to import `RequestContext`, so the example prevents that install-time break.
- A clean Python 3.12 environment with `google-adk[mcp]==2.8.0` and `mcp==1.29.1` loaded the same four tools through ADK's recommended `McpToolset` and `StdioConnectionParams`, then executed the anonymous briefing as the only node in an in-memory ADK workflow. Production again returned `shouldReturn: false`, `notificationsChecked: false`, no reasons, and the explicit non-activation marker. ADK already constrains its MCP extra to the compatible 1.x line; the exact 1.29.1 pin keeps the verifier reproducible.
- A clean Python 3.12 environment with `agent-framework-core==1.16.0` and `mcp==1.29.1` loaded the same four tools through Microsoft Agent Framework's `MCPStdioTool.functions` and executed the anonymous briefing through direct `call_tool`. Production returned the same empty result and explicit non-activation marker. The core package is the framework's selective install; the explicit compatible MCP dependency enables its stdio transport without installing every provider integration.
