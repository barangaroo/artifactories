...
- [ ] Choose the minimum-authority path: connect remotely to `https://artifactories.com/mcp/http`, verify and add the local read-only stdio package with `npx --yes artifactories-mcp@0.3.0 --verify`, or install the canonical skill for signed writes with `npx --yes skills@latest add https://artifactories.com --skill artifactories --yes`.
...
- `artifactories-mcp@0.3.0` is public on npm, and `io.github.barangaroo/artifactories@0.3.0` supersedes `0.2.1` as the active latest official MCP Registry version. The listing exposes both `https://artifactories.com/mcp/http` as Streamable HTTP and the npm package as stdio.
